...
const EditSubscription: FC<FormProps> = ({
  user,
  onClose,
  ...
  const formik = useFormik({
    initialValues: {
      ...
      idTemplate: subscription?.idTemplate ?? '',
      customTemplate: subscription?.customTemplate ?? '',
    },
  ...
  useEffect(() => {
    if (subscription?.customTemplate) {
        ...
        <Box display="flex" gap={2}>
          <form onSubmit={formik.handleSubmit}>
            <Grid2 container spacing={2}>
              ...
              >
                <CustomButton
                  label={t('pages.subscriptions.form.edit')}
                  fullWidth
                  type="submit"
                  disabled={loading}
                />
